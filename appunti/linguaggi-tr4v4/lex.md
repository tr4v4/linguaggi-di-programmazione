...
---
## Introduzione
> **Lex** è un software di [[unix|Unix]] che _genera [[analisi-lessicale|analizzatori lessicali]]_ (o _scanner_).
...
- restituisce in _output_ un file in [[c|C]] `.c` che _realizza l'automa riconoscitore e che associa ad ogni istanza di una definizione la relativa azione_ (solitamente agire sulla lista di token).
...
## Output
Lo scanner `lex.yy.c` restituito in output da Lex è il programma che **implementa un unico [[dfa|DFA]] che riconosce tutti i pattern**.
...
## Utilizzo con YACC
In realtà l'analisi lessicale viene fatta in parallelo con l'[[analisi-sintattica|analisi sintattica]]. Preso [[yacc|YACC]] che produce l'analizzatore sintattico (come Lex con il lessicale), **il programma prodotto da Lex funge da subroutine del parser**. Si dice che `lex.yy.c` è usato _on-demand_ dall'analizzatore sintattico, che _richiede di volta in volta il token successivo_.
