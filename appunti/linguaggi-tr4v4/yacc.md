...
- in input al parser e' fornita la lista di token prodotta dall'[[analisi-lessicale|analizzatore lessicale]] (tipo [[lex|Lex]]);
...
nonterm: corpo1    {azione semantica 1}
		 ...
		 ;
```
dove `{azione semantica i}` contiene il _codice [[c|C]] da eseguire nel momento in cui il parser riduce mediante quella produzione_.
