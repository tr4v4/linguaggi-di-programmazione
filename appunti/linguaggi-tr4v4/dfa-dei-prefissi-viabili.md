...
---
## Introduzione
> Il **[[dfa|DFA]] dei [[prefisso-viabile|prefissi viabili]]**, anche detto **automa canonico $LR(0)$** e' uno pseudo-DFA usato dai [[parser-bottom-up|parser bottom-up]], in particolare gli [[parser-lr|LR]], per costruire le [[tabella-di-parsing-lr|tabelle di parsing]] $LR(0)$.

Un parser ([[automa-a-pila|PDA]]) puo' consultare il DFA dei prefissi viabili (ossia la [[tabella-di-parsing-lr|tabella di parsing]] $LR(0)$ costruita a partire da esso) per decidere cosa fare:
...
### NFA
Partiamo quindi con la costruzione dell'[[nfa|NFA]] dei prefissi viabili. Fissiamo una grammatica libera $G = (NT, T, S, R)$, e aumentiamola con un nuovo simbolo iniziale $S'$ e una produzione $S' \to S$.
...
### Caso $LR(1)$
Per le [[grammatiche-lr-1|grammatiche]] $LR(1)$ sappiamo che non basta l'automa canonico $LR(0)$. **Serve per fare in modo che ogni item di ogni stato sia associato a un carattere di look-ahead**!
...
Stiamo letteralmente calcolando i follow di $X$ **per quella precisa derivazione**: infatti $\text{First}(\beta a)$ sono i follow di $X$ per l'item $LR(1)$ $[A \to \alpha . X \beta, a]$.
